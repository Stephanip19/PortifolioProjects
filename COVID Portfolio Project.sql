...
Select Location, Population, MAX(total_cases) as HighestInfectionCount,  Max(cast(total_deaths as int)/cast(total_cases as int))*100 as PercentPopulationInfected
From PortfolioProject..CovidDeaths$
--Where location like '%states%'
...
Where continent is not null 
Group by continent
order by TotalDeathCount desc as
